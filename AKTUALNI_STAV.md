...
- **Zaměstnanci – přiřazení jen do budovy/třídy (B1e):**
  - U zaměstnance volitelný výběr „Přiřazen pouze do“: žádné, konkrétní budova, nebo konkrétní třída. Pokud je nastaveno, zaměstnanec smí být v návrhu směn přiřazen jen tam (budova = libovolná třída v ní, třída = jen ta třída). Model: `zamestnanec.prirazenoJen` (null | 'b:budovaId' | 't:tridaId'). Formulář: select s možnostmi z budov a tříd; tabulka zaměstnanců: sloupec „Přiřazen pouze do“. Export/import normalizuje podle platných id budov/tříd. **Výpočet směn** respektuje omezení (canAssignEmpToLocation v assignLocations). **Validace** na záložce Návrh směn kontroluje, že segmenty zaměstnance s omezením jsou jen v povolené budově/třídě (chyba „Přiřazení jen do budovy/třídy (B1e)“).
...
- **Výpočet návrhu směn – první verze (D1) a rozšíření (D3, D5):**
  ...
  - **Typy umísťování (Pedagogové / Asistenti / Školnice):** Výpočet směn bere v úvahu **úvazek pouze pro zvolený typ umísťování**. **Pedagogové** = součet úvazků v rolích učitelka, ředitelka, zástupkyně (bez asistentky a školníka). **Asistenti** = úvazek v roli asistentka pedagoga. **Školnice** = úvazek v roli školník/školnice. Aktuálně se vždy používá umísťování **Pedagogové** (výchozí `data.typUmisteni === 'pedagogove'`); do výpočtu se zahrnou jen zaměstnanci s kladným úvazkem v těchto rolích a jejich týdenní minuty se berou jen z těchto rolí. Datový model: `TYPY_UMISTENI`, `rolePatriDoUmisteni(role, typUmisteni)`, `getUvazekMinutyZamestnanceProUmisteni(z, typUmisteni)` (js/data-model.js). UI pro přepínání typu umísťování (Asistenti, Školnice) se plánuje později.
  - **Umísťování asistentek (první krok výpočtu):** Před spuštěním pedagogického algoritmu se jako první krok umístí **asistentky**. (1) **Validace:** Každá asistentka (úvazek v roli asistentka > 0) musí mít **přiřazenu třídu** (`tridaId`); pokud nějaká nemá nebo třída v budovách neexistuje, výpočet se nespustí a zobrazí se chyba. (2) **Umístění:** Úvazek každé asistentky se rovnoměrně rozdělí do 5 dnů s zaokrouhlováním na 15 min: první den dolů na 15 min, zbytek dělíme 4/3/2 (vždy zaokrouhleno na 15 min dolů), pátý den = zbytek. Směna každý den začíná v **7:45** a trvá vypočtené denní minuty v její přiřazené třídě. (3) **Sloučení:** Výstup umísťování asistentek se sloučí s výstupem pedagogického algoritmu do jednoho návrhu. (4) **Graf:** Segmenty asistentek se v grafu zobrazují **stejnou barvou** jako má osoba v nastavení, ale **šrafované** (odlišení od pedagogických úvazků). Formulář zaměstnance: pole „Přiřazená třída“ se zobrazí i u vykrývací, pokud má asistentka roli s úvazkem; u asistentek je přiřazení třídy povinné. API: `MSemenyUmisteniAsistentek.validaceAsistentekHaveTridu(data)`, `vypocetUmisteniAsistentek(data)` (js/umisteni-asistentek.js); sloučení v `vypocetSmen` (js/vypocet-smen.js).
  - **Výpočetní logika jako pluginy (refaktor):** Výpočet je realizován jako **algoritmy s jednotným API**: každý algoritmus je objekt `{ id, nazev, vypocet(data) }`, `vypocet` vrací `{ ok, prirazeni?, chyba?, varovani? }`. **Registr** (js/algoritmy/registr.js): `MSemenyAlgoritmy.dostupneAlgoritmy()`, `getAlgoritmus(id)`, `vychoziId()`. **Základní algoritmus** (js/algoritmy/zakladni.js): celá dosavadní 4fázová logika; id `zakladni`, název „Základní (4 fáze)“. **Algoritmus „Párové třídy“** (js/algoritmy/parove-tridy.js): id `parove-tridy`, název „Párové třídy (7:45–12 / 10–16)“; na každé třídě dvojice učitelek (ideálně kmenové), dopolední 7:45–12:00, odpolední 10:00–16:00 (překryv 2 h), střídání dnů; fáze 2: ranní 7:00–7:45 a večerní 16:00–17:00 na budovu; **fáze 3a:** chybějící směny (hlavně ke konci týdne) se vykrývají zbývajícími učitelkami (bez páru), ideálně jedna učitelka = jedna třída; respektuje se `pravidla.minDelkaBlokuMinuty` (min. délka směny, výchozí 120 min); fáze 3: doplnění vykrývacími (7:00–7:45 a 16:00–17:00); varování při nevyčerpaném/přečerpaném úvazku. Jednotné 7:45–16:00 pro všechny třídy. Spec: ALGORITMUS-PAROVE-TRIDY-SPEC.md. **Facade** (js/vypocet-smen.js): `vypocetSmen(data, algorithmId?)` deleguje na vybraný algoritmus. V sekci Návrh směn **dropdown „Výpočetní logika“** – výběr mezi Základní a Párové třídy. Testy: test-algoritmy-registr.js, test-parove-tridy.js.
- **Zobrazení návrhu směn (D2):**
  - Sekce „Návrh směn“: popis, **výběr výpočetní logiky** (dropdown), tlačítko „Přepočítat“, po výpočtu tabulka se sloupci Den, Zaměstnanec, Čas, Místo. Chyba výpočtu (např. nedostatek úvazků) se zobrazí pod tlačítkem. Prázdný stav před prvním výpočtem (js/navrh-smen-ui.js).
- **Grafické zobrazení návrhu pracovní doby (D2b, D2d, D2e):**
  - Pod tabulkou návrhu: blok s grafem. Každá budova = vlastní blok; v bloku řádek „Budova (společně)“ a po řádku každá třída. Řádek = vodorovná časová osa (od otevírací doby), na ní barevné obdélníky = přiřazení osob (segmenty). Tooltip (title) u segmentu: jméno a časový rozsah (od–do). **D2e:** Barvy učitelek jsou **přiřazeny v nastavení** (Zaměstnanci → u každého pole „Barva v grafu návrhu“). Při načtení stránky nebo importu konfigurace se chybějící barvy doplní z palety a uloží. Legenda: v režimu **Taby** je legenda u každého dne; v režimu **Inline** je **jedna společná legenda** na konci. API: `MSemenyNavrhGraf.BARVY`, `doplnBarvyZamestnancum()` (volá se z main.js po nacti() a po importu). **D2d:** Místo selectu pro den jsou **záložky (taby)** Po–Pá. Tlačítko přepíná režim **Taby** vs. **Inline**. API: `MSemenyNavrhGraf.vykresliNavrhGraf(prirazeni, data, container, vybranyDen, opts)` (js/navrh-graf.js).
...
- **Validace návrhu a tabulka chyb/varování (D10, D10b):**
  - Tlačítko „Validovat“ zkontroluje aktuální návrh (vygenerovaný nebo načtený z CSV). Zobrazí tabulku s porušenými pravidly a kontextem. **Přečerpaný/nevyčerpaný úvazek (D10)** se kontroluje **v rámci role (typu umístění)** – jeden zaměstnanec může mít např. přečerpaný úvazek učitelka a nevyčerpaný asistentka; položky v tabulce uvádějí typ (např. „(asistentka)“). **D10b – min/max na třídu a budovu:** kontrola se aplikuje **jen nad pedagogickými úvazky** (segmenty s typem asistenti nebo skolnice se do počtu osob pro min/max nezapočítávají). Ostatní kontroly (překryv, B1e, „na dvou místech zároveň“) platí napříč všemi typy. API: `MSemenyValidaceNavrhu.validujNavrh(prirazeni, data)` (data obsahuje zamestnanci, budovy, minMaxSloty); `sumMinutyPerZamestnanecPoType`, `segmentPatriDoMinMaxPedagog`.
- **Editace bloků přímo na grafu (D11):**
  - Klik na segment v grafu otevře modal „Upravit blok přiřazení“: **typ úvazku** (Pedagog / Asistentka / Školnice) – u zaměstnanců s více úvazky lze vybrat, který úvazek blok reprezentuje; změna času od–do; přesun do jiné třídy/budovy (select); tlačítko Smazat blok. Přetažení (drag & drop) segmentu na jiný řádek grafu přesune blok do příslušné třídy nebo budovy. Po uložení nebo přesunu se aktualizuje tabulka návrhu, graf a (je-li zobrazena) tabulka validace. API: `MSemenyNavrhGraf.vykresliNavrhGraf(..., opts)` s `onSegmentClick` a `onSegmentDrop`; `MSemenyNavrhSmenUI.applyNavrhChange(prirazeni)`.
- **Doplnění nevyčerpaného úvazku z tabulky chyb (D12):**
  - V tabulce validace je u položky „Nevyčerpaný úvazek“ a „Přečerpaný úvazek“ jméno zaměstnance klikatelné. Klik otevře modal „Doplnit úvazek“: **typ úvazku** (u zaměstnanců s více úvazky výběr Pedagog / Asistentka / Školnice; při otevření z chybové tabulky se předvyplní typ, kterého se chyba týká); výběr dne, času od, třídy. **Čas do** se dopočítá podle zbývajícího času **pro vybraný typ úvazku** (min do úvazku v této roli, max 8 h, omezeno otevírací dobou třídy). Po uložení se přidá nový blok s příslušným `typUmisteni` a zobrazení včetně validace se obnoví.
- **Chybový report pro AI (D10c):**
  - Po kliku na „Přepočítat“ se automaticky spustí validace. Jsou-li nalezeny chyby nebo varování, zobrazí se tlačítko „Stáhnout chybový report pro AI“. Report (soubor .md) obsahuje **anonymizovanou konfiguraci** (zaměstnanci jako ucitelka1, reditelka1, …; budovy a třídy jako budova1, trida1, …), **vygenerovaný přehled** (přiřazení s těmito labely) a **nalezené chyby**. Slouží k předání AI k nápravě. Dokumentace **CHYBOVY_REPORT.md** popisuje obsah reportu a doporučený postup: 1) napsat test, který chybu odhalí, 2) upravit kód tak, aby test prošel. API: `MSemenyChybovyReport.generateReport`, `stahnoutChybovyReport` (js/chybovy-report.js).
...
- 2025-02-06: Validace a UI po typech úvazku: (1) Přečerpaný/nevyčerpaný úvazek se kontroluje per role (typ umístění); (2) Min/max na třídu a budovu jen nad pedagogickými úvazky (asistenti/školnice se ignorují); (3) Modal „Upravit blok“ a „Doplnit úvazek“ – výběr typu úvazku u zaměstnanců s více úvazky; při otevření z chyb předvyplnění typu z chyby; „Čas do“ v doplnit modalu počítán pro vybraný typ. js/validace-navrhu.js (sumMinutyPerZamestnanecPoType, segmentPatriDoMinMaxPedagog), js/navrh-smen-ui.js (openDoplnitModal, openSegmentEditModal, saveSegmentEdit, saveDoplnitUvazek, updateDoplnitCasDo).
- 2026-02-09: Algoritmus Párové třídy – fáze 3a: doplnění chybějících směn (dopolední/odpolední) zbývajícími učitelkami, které nejsou v páru; preferuje přiřazení do jedné třídy; respektuje `pravidla.minDelkaBlokuMinuty` (výchozí 2 h). Spec doplněna o Fáze 3a, testy v test-parove-tridy.js.
- 2026-02-06: Implementován D10c – po Přepočítat automaticky validace; při chybách tlačítko „Stáhnout chybový report pro AI“, anonymizovaný report (konfigurace, přehled, chyby), CHYBOVY_REPORT.md (postup pro AI: test → oprava kódu).
- 2026-02-06: Implementován D2d – graf návrhu: záložky (taby) místo selectu pro den; tlačítko přepíná režim Taby (jeden den) vs. Inline (všechny dny pod sebou). navrh-graf.js: buildObsahJednohoDne, ovládací prvky; navrh-smen-ui.js: stav režim/den a callbacky; CSS pro taby a režim.
- 2026-02-06: Implementován B1e – zaměstnanci: přiřazení jen do konkrétní budovy/třídy. Model prirazenoJen (null | 'b:budovaId' | 't:tridaId'), formulář select, tabulka sloupec, export/import normalizace. Výpočet směn respektuje omezení (canAssignEmpToLocation). Validace návrhu kontroluje B1e.
