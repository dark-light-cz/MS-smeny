...
- **Výpočet návrhu směn – první verze (D1) a rozšíření (D3, D5):**
  ...
  - **Výpočetní logika jako pluginy (refaktor):** Výpočet je realizován jako **algoritmy s jednotným API**: každý algoritmus je objekt `{ id, nazev, vypocet(data) }`, `vypocet` vrací `{ ok, prirazeni?, chyba?, varovani? }`. **Registr** (js/algoritmy/registr.js): `MSemenyAlgoritmy.dostupneAlgoritmy()`, `getAlgoritmus(id)`, `vychoziId()`. **Základní algoritmus** (js/algoritmy/zakladni.js): celá dosavadní 4fázová logika; id `zakladni`, název „Základní (4 fáze)“. **Facade** (js/vypocet-smen.js): `vypocetSmen(data, algorithmId?)` deleguje na vybraný algoritmus; bez `algorithmId` se použije výchozí. V sekci Návrh směn je **dropdown „Výpočetní logika“** – aktuálně jedna možnost (Základní), připraveno pro budoucí další algoritmy. Testy: test-algoritmy-registr.js.
...
- **Grafické zobrazení návrhu pracovní doby (D2b, D2d):**
  - Pod tabulkou návrhu: blok s grafem. Každá budova = vlastní blok; v bloku řádek „Budova (společně)“ a po řádku každá třída. Řádek = vodorovná časová osa (od otevírací doby), na ní barevné obdélníky = přiřazení osob (segmenty). Tooltip (title) u segmentu: jméno a časový rozsah (od–do). Legenda barev (kdo jakou barvu má). **D2d:** Místo selectu pro den jsou **záložky (taby)** Po–Pá. Tlačítko přepíná režim **Taby** (jeden den na obrazovce, přepínání záložkami) vs. **Inline** (všechny dny pod sebou na jedné stránce). API: `MSemenyNavrhGraf.vykresliNavrhGraf(prirazeni, data, container, vybranyDen, opts)` s `opts.rezim` ('taby'|'inline'), `opts.onDenChange`, `opts.onRezimChange` (js/navrh-graf.js).
