...
| D2b | **Grafické zobrazení návrhu pracovní doby** – zobrazit výsledný návrh pracovní doby graficky. Struktura: každá budova = vlastní blok; v bloku každá třída = vlastní řádek. Řádek = časová osa (např. vodorovně), na ní barevné obdélníky = jednotlivé učitelky (přiřazení v daném čase). Při najetí myší (popup/tooltip) zobrazit od kdy do kdy (časový rozsah) a jméno učitelky. Volitelně: legenda barev (kdo jakou barvu má), zobrazení po dnech (jeden den = jedna „síť“ budov/tříd). | [ ] | |
